//! Auto-generated C FFI for the identity_lab program.
...
pub enum IdentityLabInstruction {
    VerifyAgeProof {
        proof: Vec<u8>,
        min_age: u8,
    },
...
/// FFI: verify_age_proof instruction.
#[no_mangle]
pub extern "C" fn identity_lab_verify_age_proof(args_json: *const c_char) -> *mut c_char {
    let args = match cstr_to_str(args_json) {
        Ok(s) => s, Err(e) => return error_json(&e),
    };
    match identity_lab_verify_age_proof_impl(args) {
        Ok(r) => to_cstring(r), Err(e) => error_json(&e),
    }
}

fn identity_lab_verify_age_proof_impl(args: &str) -> Result<String, String> {
    ...
    let proof = v["proof"].as_array().ok_or("expected array")?.iter().map(|item| Ok(item.as_u64().ok_or("expected number")? as u8)).collect::<Result<Vec<_>, String>>()?;
    let min_age = v["min_age"].as_u64().ok_or("expected number")? as u8;
    ...
    let instruction = IdentityLabInstruction::VerifyAgeProof {
        proof,
        min_age,
    };
...
#[no_mangle]
pub extern "C" fn identity_lab_free_string(s: *mut c_char) {
    if !s.is_null() { unsafe { drop(CString::from_raw(s)) }; }
}

#[no_mangle]
pub extern "C" fn identity_lab_version() -> *mut c_char {
